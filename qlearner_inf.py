...
class Qlearner_INF:
    def __init__(self, data, verbose=False, model='forest', epoch=1, 
                 rbf_dim=5, use_mediator=True, eps=1e-2):
        '''
        ...
        self.reward = np.reshape(self.reward, (-1, 1))
        ...
        if self.model == "linear":
            ...
            if self.use_mediator:
                ...
                if X_mediator.ndim == 1:
                    X_mediator = X_mediator.reshape(-1,
                                                    self.mediator_dummy_size)
    ...
    def fit(self):
        ...
        for i in range(self.epoch):
            ...
            self.one_batch_fit()
            if i >= 1:
                score_difference = self.score_array[i] - \
                    self.score_array[i - 1]
                rmse_difference = self.rmse_array[i] - self.rmse_array[i - 1]
                if self.model == "linear":
                    coef_diff1 = self.q_model_list[i].coef_ - \
                        self.q_model_list[i - 1].coef_
                    coef_diff1 = np.linalg.norm(coef_diff1, ord=1)
                    coef_diff2 = np.abs(
                        self.q_model_list[i].intercept_ - self.q_model_list[i - 1].intercept_)
                    coef_diff = coef_diff1 + coef_diff2
                    coef_norm = np.linalg.norm(
                        self.q_model_list[i].coef_, ord=1)
    ...
    def goodness_of_fit(self, target_policy, new_state, new_action, new_mediator, new_reward, new_next_state):
        np.random.seed(1)
        new_policy_action = np.apply_along_axis(
            target_policy, 1, new_next_state)
